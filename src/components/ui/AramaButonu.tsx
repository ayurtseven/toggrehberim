  ...
  return (
    <>
      ...
      <button
        ...
        className="flex items-center gap-2 rounded-lg border border-white/20 px-3 py-2 text-sm text-neutral-400 transition-colors hover:border-white/40 hover:text-neutral-200"
        aria-label="Ara"
      >
        ...
        <kbd className="hidden rounded border border-white/15 bg-white/8 px-1.5 py-0.5 text-[10px] font-medium text-neutral-600 lg:inline">
          ⌘K
        </kbd>
      </button>
      ...
      {acik && (
        ...
        >
          ...
          {/* Modal kutusu */}
          <div className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-white/12 bg-neutral-900 shadow-2xl shadow-black/60">
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/10">
              {yukleniyor ? (
                ...
                <svg className="h-5 w-5 shrink-0 text-neutral-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              )}
              <input
                ...
                onKeyDown={onKeyDown}
                className="flex-1 bg-transparent text-base text-white placeholder:text-neutral-600 outline-none"
              />
              <button onClick={kapat} className="shrink-0">
                <kbd className="rounded border border-white/15 bg-white/8 px-1.5 py-0.5 text-[10px] font-medium text-neutral-500 cursor-pointer hover:bg-white/15">
                  ESC
                </kbd>
              </button>
            </div>
            ...
            <div className="max-h-[55vh] overflow-y-auto">
              {sorgu && sonuclar.length > 0 ? (
                    ...
                    <li key={item.href}>
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="truncate text-xs text-neutral-500">{item.ozet}</p>
                        </div>
                ...
                <div className="px-4 py-4">
                  <p className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-neutral-600">
                    Hızlı Erişim
                  </p>
                  <div className="space-y-1">
                    {HIZLI_LINKLER.map((link) => (
                      ...
                      >
                        ...
                        <span className="text-sm text-neutral-300">{link.label}</span>
                      </Link>
                    ))}
                  </div>
                </div>
            ...
            {/* Footer */}
            <div className="flex items-center gap-4 border-t border-white/8 px-4 py-2.5 text-[11px] text-neutral-700">
              <span><kbd className="font-mono text-neutral-600">↑↓</kbd> gezin</span>
              <span><kbd className="font-mono text-neutral-600">↵</kbd> git</span>
              <span><kbd className="font-mono text-neutral-600">esc</kbd> kapat</span>
              {veri && (
